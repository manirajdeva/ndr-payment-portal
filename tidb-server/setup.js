...
async function main() {
  ...
  // Optional starter employee login, seeded with the 'employee' role.
  // EMPLOYEE_* is preferred; HR_* is still read as a fallback so older .env
  // files keep working — this always seeds 'employee', never the (now
  // distinct) 'hr' role. More users, including 'hr' ones, are added from
  // the app itself: Settings > User Management (admin only).
  await seedUser(
