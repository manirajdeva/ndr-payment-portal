...
- **Job Status** — pick an existing student (search by name/ID/mobile), which auto-fills their name and course; track status through the fixed dropdown (`Enrolled` → `Training` → `Scheduling Interview` → `Interview Cleared` → `Offer Received` → `Job Joined`, plus `Rejected` and `In-active`). Records saved under the older status names (`Pending`, `Interview Scheduled`, `Selected`, `Joined`) are mapped onto the current ones automatically, and the TiDB backend rewrites them in place on startup.
...
- **Audit log** (TiDB backend) — every add, edit, and delete on Students, Job Status and Payments is recorded in `students_log` / `jobs_log` / `payments_log` with the acting username, a before/after snapshot, and a timestamp. Read it via the `getAuditLog` action or straight from the tables. See [tidb-server/README.md](tidb-server/README.md#audit--history-log).
- **Settings** — light/dark theme toggle, account info, and **User Management** (admins only): add, edit, or remove portal logins. Each login is either an **admin** (full access, including Payments and managing users) or an **employee** (full add/edit/delete on Student Enquiries and Job Status, plus the Dashboard and Reports, but **no access to the Payments section** and cannot manage users). New users and password resets are done here; only the primary bootstrap admin's password is still rotated at the backend (see 2.2 / the TiDB README).
