...
- **Settings** — light/dark theme toggle, account info, and **User Management** (admins only): add, edit, or remove portal logins. Each login is either an **admin** (full access, including managing users) or an **employee** (can view every section and add new records, but cannot edit or delete anything). New users and password resets are done here; only the primary bootstrap admin's password is still rotated at the backend (see 2.2 / the TiDB README).
